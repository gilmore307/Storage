...
pyinstaller --noconsole --onefile --clean --name $ProfileName --distpath dist --workpath build-work --specpath build-spec --icon resources/OpenClaw.jfif --add-data "$ProfileName.profile.json;." openclaw_node_tray_app.py --hidden-import pystray --hidden-import PIL --hidden-import psutil
